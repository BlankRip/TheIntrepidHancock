...
public class PlayerMovement : MonoBehaviour
{
    ...
    public void Movement(float horizontalMove, float verticalMove, float speed, bool crouch)
    {
        ...
        if (horizontalMove > 0 && horizontalMove < moveThreshHold)
            transform.localRotation = Quaternion.EulerAngles(transform.localRotation.x, transform.localRotation.y + 90, transform.localRotation.z);
        else if (horizontalMove < 0 && horizontalMove > -moveThreshHold)
            rb.rotation = Quaternion.EulerAngles(transform.localRotation.x, transform.localRotation.y - 90, transform.localRotation.z);
        else if (verticalMove > 0 && verticalMove < moveThreshHold)
            rb.rotation = Quaternion.EulerAngles(transform.localRotation.x, transform.localRotation.y - 180, transform.localRotation.z);
        else if (verticalMove < 0 && verticalMove > -moveThreshHold)
            rb.rotation = Quaternion.EulerAngles(transform.localRotation.x, transform.localRotation.y + 180, transform.localRotation.z);
    }
}
