...
//#include "Support.cginc"
	...
	//	sampler2D _MainTex;
		...
		float AmbiantFactor(float2 screenPos) 
		{
			...
			float linear01 = 1 - Linear01Depth (screenSimpleDepth);
			...
			for (float i = 1; i < SampleCount; i++)
			{
				...
				float shiftSimpleDepth = tex2D(_CameraDepthTexture, screenPos + (screenRay.xy * _ScanDistance * (linear01) * _SampleShift)).r;
